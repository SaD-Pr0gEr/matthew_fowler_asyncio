...
Сопрограммы цикла событий для сокетов
    ...
    Методы sock_recv и sock_sendall названы по аналогии с sock_ac-
    cept. Они принимают сокет и ждут результата. sock_recv ждет по-
...
Проектирование асинхронного эхо-сервера
    Итак, сопрограмма для прослушивания порта у нас есть, а как на-
    счет чтения и записи данных подключившимся клиентам? Для это-
    го нужна просто сопрограмма или сопрограмма, обернутая задачей?
    ...
    остальные, желательно иметь возможность читать и записывать дан-
    ные для нескольких клиентов конкурентно. Поскольку требуется об-
    рабатывать несколько подключений одновременно, имеет смысл
    ...
    каждого нового подключения будем создавать задачу, обертываю-
    щую сопрограмму echo. Эти две сопрограммы дают все, что нужно для
